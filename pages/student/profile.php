...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($f['email'] !== '') {
        ...
        elseif (User::emailExists($f['email'], (int) ($student['user_id'] ?? 0))) $errors['email'] = 'Email sudah digunakan.';
    }
    if ($f['password'] !== '' && strlen($f['password']) < 6) $errors['password'] = 'Password minimal 6 karakter.';

    $photo = $student['photo'] ?? 'default.png';
    if (isset($_FILES['photo'])) {
        $res = handlePhotoUpload($_FILES['photo'], $photo, 'student_');
    ...
    if (empty($errors)) {
        ...
        $db->beginTransaction();
        try {
            Student::update($student['id'], array_merge([
                'nis'        => $student['nis'],
            ...
            if ($student['user_id'] && $f['email'] !== '') {
                User::update((int) $student['user_id'], ['name' => $f['name'], 'email' => $f['email'], 'password' => $f['password']]);
            }

            $db->commit();
            $_SESSION['user_name'] = $f['name'];
            flash('success', 'Profil berhasil diperbarui.');
            header('Location: ' . BASE_URL . '/pages/student/profile.php');
            exit;
        } catch (Exception $e) {
            $db->rollBack();
...
$breadcrumb = [['label' => 'Portal Siswa'], ['label' => 'Profil']];
require_once APP_ROOT . '/includes/header.php';

$hp       = UPLOAD_PATH . ($student['photo'] ?? '');
$hasPhoto = ($student['photo'] ?? 'default.png') !== 'default.png' && file_exists($hp);
$initials = strtoupper(mb_substr($student['name'], 0, 2));
?>
...
<!-- Profile hero -->
<div class="ns-glass-panel p-6 mb-6 relative overflow-hidden" style="background: linear-gradient(135deg, var(--ns-role-student), #BE123C); border: none;">
    <div class="flex flex-col sm:flex-row items-center gap-6 relative z-10">
        ...
        <div class="text-center sm:text-left text-white">
            <p class="font-serif text-3xl font-normal leading-tight mb-1"><?= htmlspecialchars($student['name']) ?></p>
            <p class="text-sm text-white/90 font-medium mb-0.5">NIS <?= htmlspecialchars($student['nis']) ?></p>
            <?php if (!empty($student['class_name'])): ?>
            <p class="text-xs text-white/70">Kelas <?= htmlspecialchars($student['class_name']) ?></p>
            <?php endif; ?>
        </div>
    </div>
    ...
    <div class="ns-form-body">
        <div class="ns-form-group">
            ...
            <input type="file" name="photo" accept="image/jpeg,image/png,image/gif,image/webp"
                   class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-rose-50 file:text-rose-700 hover:file:bg-rose-100 transition-colors cursor-pointer">
    ...
    <div class="ns-form-body">
        ...
        <div class="ns-form-group" style="margin-bottom:16px;">
            ...
            <textarea name="address" rows="3" placeholder="Jl. …"
                      class="ns-form-input w-full resize-y"><?= htmlspecialchars($f['address']) ?></textarea>
        </div>
...
<div class="flex items-center justify-end gap-3 mb-8">
    <a href="<?= BASE_URL ?>/pages/student/dashboard.php" class="ns-btn ns-btn-secondary">Batal</a>

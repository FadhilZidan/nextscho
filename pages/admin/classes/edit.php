...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        getDB()->prepare('UPDATE classes SET name=?, grade_level=?, academic_year=?, homeroom_teacher_id=? WHERE id=?')
            ->execute([$f['name'], $f['grade_level'], $f['academic_year'], $f['homeroom_teacher_id'] ?: null, $id]);
        flash('success', "Kelas {$f['name']} berhasil diperbarui.");
        header('Location: ' . BASE_URL . '/pages/admin/classes/index.php');
        exit;
    }
}
...
<div class="max-w-lg" data-animate="fade-up">
    ...
    <div class="ns-page-header" style="margin-bottom:20px;">
        <div>
            <h1 class="ns-page-title">Edit Kelas</h1>
            <p class="ns-page-subtitle"><?= htmlspecialchars($kelas['name']) ?> · Tingkat <?= htmlspecialchars($kelas['grade_level']) ?></p>
        </div>
    </div>
